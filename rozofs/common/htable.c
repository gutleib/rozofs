...
*  Init of the hash table for multi thread environment
   ...
   @retval < 0 error (see errno for details)
*/
int htable_initialize_th_with_copy(htable_t * h, uint32_t size,uint32_t lock_size, uint32_t(*hash) (void *),
                       int (*cmp) (void *, void *),void (*copy) (void *, void *)) {
    list_t *it;
    int i;
    int ret;
    DEBUG_FUNCTION;
...
int htable_get_copy_th(htable_t * h, void *key,uint32_t hash,void *dest) {
    ...
    */
    errno = EFAULT;
    if (h->copy == NULL) return -1;
    pthread_rwlock_rdlock(&h->lock[hash%h->lock_size]);
    list_for_each_forward(p, h->buckets + (hash % h->size)) {
        hash_entry_t *he = list_entry(p, hash_entry_t, list);
        if (h->cmp(he->key, key) == 0) {
            h->copy(he->value,dest);
            pthread_rwlock_unlock(&h->lock[hash%h->lock_size]);
            return 1;
        }
    }
