...
{
	char tmp[128];
	const char *devname_fuse = "/dev/fuse";
        const char *devname_rozo = "/dev/rozo";
	char *devname = NULL;
	int rozo_device_flag = 0;
	char *source = NULL;
	char *type = NULL;
	struct stat stbuf;
	int fd;
...
//		fprintf(stderr ,"MOUNTING FUSE !!!!\n");
	
	/*
	**______________________________________________
	** Check if the device rozo is up and running
	**______________________________________________
	*/
        res = stat(devname_rozo, &stbuf);
	if (res == -1)
	{
          rozo_device_flag = 0; 
	  devname = (char*)devname_fuse;	
	}
	else
	{
	
          rozo_device_flag = 1; 
	  devname = (char*)devname_rozo;		
	}
	/*
	** End check rozo module
	*/
	if (!mnt) {
		fprintf(stderr, "fuse: missing mountpoint parameter\n");
		return -1;
	...
	if (!mo->nonempty) {
	        /*
		** Depends on the device that has been selected
		*/
		res = fuse_mnt_check_empty((rozo_device_flag!=0)?"rozo":"fuse", mnt, stbuf.st_mode,
					   stbuf.st_size);
		if (res == -1)
			return -1;
	}
	...
	if (fd == -1) {
		if (errno == ENODEV || errno == ENOENT)
			fprintf(stderr, "ROZO_fuse_01 (%s:%s): device not found, try 'modprobe fuse' first\n",devname,strerror(errno));
		else
	...
	/*
	**______________________________________________
	** Check if the device rozo is in use
	**______________________________________________
	*/
	if (rozo_device_flag == 0)
	{
	  strcpy(type, mo->blkdev ? "fuseblk" : "fuse");
        }
	else
	{
	  strcpy(type, mo->blkdev ? "rozoblk" : "rozo");
	}
	if (mo->subtype) {
		strcat(type, ".");
...
#ifndef IGNORE_MTAB
	if (geteuid() == 0) {
		char *newmnt = fuse_mnt_resolve_path("rozo", mnt);
		res = -1;
		if (!newmnt)
			goto out_umount;

		res = fuse_mnt_add_mount((rozo_device_flag == 0)?"fuse":"rozo", source, newmnt, type,
					 mnt_opts);
		free(newmnt);
		if (res == -1)
			goto out_umount;
