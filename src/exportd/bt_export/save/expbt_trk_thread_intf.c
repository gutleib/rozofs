...
int       expbt_recv_sockpair[2];       /**< index 0 is used by the RDMA signaling thread, index 1 is used by the client */
...
rozorpc_srv_ctx_t *rozorpc_srv_ctx_p = NULL;
  ...
  */
  while(1) {  
    ...
    */
    bytesRcvd = read(expbt_recv_sockpair[1],&rozorpc_srv_ctx_p,sizeof(rozorpc_srv_ctx_t *));
    if (bytesRcvd == -1) {
     switch (errno)
     {
       case EAGAIN:
...
void af_unix_trk_pool_socket_on_receive_buffer_depletion()
{
    ...
    */   
    ...
    {        
      ...
      ** wait for event 
      */
      FD_SET(expbt_recv_sockpair[1],&localRdFdSet);   	  
      /*
      ...
      */
      ...
      nbrSelect=select(expbt_recv_sockpair[1]+1,(fd_set *)&localRdFdSet,(fd_set *)NULL,NULL, &timeout);
      if (nbrSelect < 0) 
      {
         if (errno == EINTR) 
	 {
      ...
      */
      af_unix_trk_rcvMsgsock(NULL,expbt_recv_sockpair[1]);
      /*
...
int expbt_send2mainthread(void *rpc_ctx_p)
{
   ...
   ret = write(expbt_recv_sockpair[0],rpc_ctx_p,sizeof(rpc_ctx_p));
   if (ret < 0) return -1;
   if (ret != sizeof(rpc_ctx_p)) {
     fatal("expbt_send2mainthread  write error on  expbt_recv_sockpair: %s",strerror(errno));
     exit(0);  
   }
   return 0;
}
...
*/
int expbt_trk_create_receive_socket()
{
  int fileflags;
  /* 
  ** create the socket pair
  */
  if (socketpair(  AF_UNIX,SOCK_DGRAM,0,&expbt_recv_sockpair[0])< 0)
  {
  ...
  while(1)
  {
    ...
    */
    if((fileflags=fcntl(expbt_recv_sockpair[1],F_GETFL,0))<0)
    {
      warning ("expbt_recv_sockpair[1]fcntl error:%s",strerror(errno));
      break;
    }
    if (fcntl(expbt_recv_sockpair[1],F_SETFL,(fileflags|O_NDELAY))<0)
    {
      warning ("expbt_recv_sockpair[1]fcntl error:%s",strerror(errno));
      break;
    }
  ...
  */
  expbt_recv_sockctrl_p = ruc_sockctl_connect(expbt_recv_sockpair[1],
                                	   "EXP_BT_TH_RSP",
                                	    16,
                                	    NULL,
...
int expbt_trk_thread_create(char * hostname, int nb_threads) {
   ...
   */
   ...
   for (i = 0; i < nb_threads ; i++) {
     ...
     thread_ctx_p->sendSocket = expbt_recv_sockpair[0];
...
*  @retval 0 on success -1 in case of error
*/
int expbt_trk_thread_intf_create(char * hostname, int nb_threads,int cmdring_size) {

  int ret;

  af_unix_trk_thread_count = nb_threads;
  
  ret = expbt_trk_create_receive_socket();
  if (ret < 0) return ret;
