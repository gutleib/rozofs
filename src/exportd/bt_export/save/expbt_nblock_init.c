...
void show_profiler(char * argv[], uint32_t tcpRef, void *bufRef) {
    ...
    pChar += sprintf(pChar, "--------------+-----------------+-----------+--------------------+-----------------+\n");
    SHOW_PROFILER_PROBE(file_check);
...
uint32_t ruc_init(uint32_t test, expbt_start_conf_param_t *arg_p) {
    ...
    while (1) {
	   ...
	   uma_dbg_init_no_system(10, INADDR_ANY, arg_p->debug_port);
        }                
        {
            char name[256];
            char * pChar = name;
            
            pChar += sprintf(pChar, "expbt %d ", arg_p->instance);
...
int expbt_start_nb_th(void *args) {
  ...
  */	
  ret = expbt_trk_thread_intf_create(NULL,common_config.nb_expbt_thread,common_config.expbt_buf_cnt) ;
  if (ret < 0) {
