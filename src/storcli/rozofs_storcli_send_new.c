                              ...
                              sys_recv_pf_t recv_cbk,void *user_ctx_p) 
{
    ...
    */
    ...
    {
      if (!north_lbg_is_local(lbg_id) && (storcli_lbg_is_rdma_up(lbg_id,&rdma_socket_ref)))
      {
	 switch (opcode) 
	 {
	   /*
      ...
      ** Standalone case
      */
      if ((common_config.standalone) && (north_lbg_is_local(lbg_id)))
      {
	if (storcli_lbg_is_standalone_up(lbg_id,&rdma_socket_ref))
	{  
	  switch (opcode) 
	  {
	    /*
...
/*
** next entry point when compiled with RDMA
*/
tcp_mode:
#endif
